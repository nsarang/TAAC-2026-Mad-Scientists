...
This dataset uses a **flat column layout** — all features are stored as individual top-level columns
instead of nested structs/arrays.
...
> **Note:** When `user_int_feats_{fid}` and `user_dense_feats_{fid}` share the same `{fid}`,
> they are aligned and jointly describe the same entity or signal.
